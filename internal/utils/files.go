	...
	func PrepareSubmissionFiles(submissionID, code, problemID string) error {
		...
		// Write main.cpp
		err = os.WriteFile(filepath.Join(basePath, "main.cpp"), []byte(code), 0644)
		if err != nil {
			return fmt.Errorf("failed to write main.cpp: %v", err)
		}
